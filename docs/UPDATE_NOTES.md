...
### 상대수익률 추이 원형 차트로 개편
- 라벨이 서로 겹쳐 알아보기 어렵던 문제를 개선하기 위해 상대수익률 차트를 원형 레이아웃으로 다시 만들었습니다. 중심=KOSPI(상대수익률 0)를 기준으로 동심원·십자축을 두고, 각 업종의 궤적과 화살표를 원 안에 그립니다.
- 업종 라벨은 끝점 위치에 따라 좌/우로 나눠 원 밖에 세로로 정렬하고, 각 라벨을 얇은 리더선(꼬리표)으로 해당 지점과 연결했습니다. 라벨 간 최소 간격을 확보해 서로 겹치지 않습니다.
